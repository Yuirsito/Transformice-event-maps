...
tfm.exec.setUIMapName("Transformice<bl> - @2003")
...
tfm.exec.respawnPlayer(player)
if p[player].team=="red" then
tfm.exec.setNameColor(player, 0xff0000)
else
tfm.exec.setNameColor(player, 0x0000ff)
end
end

function eventPlayerRespawn(name)
if p[name].team=="red" then
tfm.exec.movePlayer(name,40,350,false,0,0,false)
else
tfm.exec.movePlayer(name,5360,350,false,0,0,false)
end
end
...
tfm.exec.setNameColor(name, colors[ i ])
if (i == #colors) then
i = 1 
p[name]={team="blue"}
tfm.exec.movePlayer(name,5360,350,false,0,0,false)
...
for i,n in ipairs(huggers) do if n == playerName then table.remove(huggers) end end
if emoteId == 17 then
table.insert(huggers, playerName)
end
if emoteId == 18 or emoteId == 19 then
for i,n in ipairs(huggers) do
local x1 = tfm.get.room.playerList[n].x
local y1 = tfm.get.room.playerList[n].y
local x2 = tfm.get.room.playerList[playerName].x
local y2 = tfm.get.room.playerList[playerName].y
if math.sqrt((x2 - x1)^2 + (y2 - y1)^2) <= 30 then
if p[n].team=="red" and p[playerName].team=="blue" then
tfm.exec.giveCheese(n)
tfm.exec.giveCheese(playerName)
tfm.exec.playerVictory(n)
tfm.exec.playerVictory(playerName)
table.remove(huggers, i)
elseif p[n].team=="blue" and p[playerName].team=="red" then
tfm.exec.giveCheese(n)
tfm.exec.giveCheese(playerName)
tfm.exec.playerVictory(n)
tfm.exec.playerVictory(playerName)
